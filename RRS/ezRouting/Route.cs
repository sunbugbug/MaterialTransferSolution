...
namespace ezRouting
{
        ...
        public void useRoute(Graph g)
        {
            for (int i = 0; i < _route.Count; i++)
            {
                ...
                if (i < _route.Count - 1)
                {
                    ...
                    {
                        if (e.to.Equals(_route[i + 1])) { 
                            e.EdgeUse();
                            edgeUseFlag = true;
                        }
                    }
                    if (!edgeUseFlag) throw new RouteException("Use Routed Edge Exception."); 
    ...
    public class AtomicRoute : Route
    {
        public AtomicRoute(string vertex) : base(vertex)
        {
